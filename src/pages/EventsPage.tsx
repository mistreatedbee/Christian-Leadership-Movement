...
export function EventsPage() {
  ...
  const handleRegister = async (eventId: string) => {
    if (!user) {
      navigate('/login', { state: { returnTo: '/events' } });
      return;
    }
    ...
    try {
      ...
      // Update local state
      setUserRegistrations({ ...userRegistrations, [eventId]: true });
      setRegistrations({ ...registrations, [eventId]: (registrations[eventId] || 0) + 1 });
