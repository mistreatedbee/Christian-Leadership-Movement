...
import { Plus, Edit, Trash2, Upload, X, Save, FileText, Video, Headphones, Link as LinkIcon, BookOpen, FileCheck } from 'lucide-react';
...
export function MembershipManagementPage() {
  const { user } = useUser();
  const [resources, setResources] = useState<MembershipResource[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingResource, setEditingResource] = useState<MembershipResource | null>(null);
  ...
  const fetchResources = async () => {
    try {
      ...
      setMessage({ type: 'error', text: err.message || 'Failed to fetch resources' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (resource: MembershipResource) => {
    ...
    try {
      ...
      if (resourceFile) {
        const { data: uploadData, error: uploadError } = await insforge.storage
          .from('resources')
          .upload(`membership/${Date.now()}_${resourceFile.name}`, resourceFile);
      ...
      if (thumbnailFile) {
        const { data: uploadData, error: uploadError } = await insforge.storage
          .from('resources')
          .upload(`membership/thumbnails/${Date.now()}_${thumbnailFile.name}`, thumbnailFile);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-navy-ink mb-2">Membership Management</h1>
          <p className="text-gray-600">Manage resources, materials, and links for the Membership program</p>
        </div>
          ...
          <div className="bg-white rounded-card shadow-soft p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-navy-ink">
                {editingResource ? 'Edit' : 'Create'} Membership Resource
              </h2>
